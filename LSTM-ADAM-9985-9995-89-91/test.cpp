...
int main(){
    ...
    */
    ...
    for(int i = 0; i < steps_per_cycle * number_of_cycles; i++)
      {
	// cout << "HIT:" << sin_raw_data[i] << endl;
	outputfile<< sin_raw_data[i] << endl;
      }
    outputfile.close();
    ...
    */
    ...
    std::ofstream writing_file_log;
    writing_file_log.open(log_filename, std::ios::out);
    ...
    for (int i=0; i<jump * epoch_size; i++){
        ...
        if ((i+1) % 1000 == 0){
            ...
            cout << "iter:" << (i+1) << "/jump:" << jump << "/epoch:" << epoch << "/total iters:" << jump * epoch_size
	    	    ...
	    	    writing_file_log << "iter:" << (i+1) << "/jump:" << jump << "/epoch:" << epoch << "/total iters:" << jump * epoch_size
                 << " " << ((float) (i+1))/(float(jump * epoch_size))*100 << "%"
                 << " loss:" << cur_log_perp/1000
		 << " iters/sec:" << ((float)1000)/((float)elapsed) << " timesec:" << elapsed << endl;
    ...
    */
	...
	writing_file << sin_raw_data[i+1] << "," << final_pre << endl;
	...
	writing_file << (sin_raw_data[input_seq + j]) << "," << final_pre << endl;
    }

    clock_t endc = clock();     // 終了時間
    writing_file_log << "total duration = " << (double)(endc - startc) / CLOCKS_PER_SEC << "sec | "
	      << (double)(endc - startc) / CLOCKS_PER_SEC / 60 << "min " << endl;

    writing_file.close();
    writing_file_log.close();
